...
```{r, echo=FALSE,results=TRUE, warning=FALSE, message=FALSE, fig.align='center', height=8, width=7.5}
otherPipDeletions <- read_csv('For Reviewers/PipDeletionsAllLineages.csv')
completeDeletionsRaw <- inner_join(Data_S4,otherPipDeletions)

completeDeletions <- completeDeletionsRaw %>% gather("carbonSource","grRatio", -gene,-strain)
# Filter out carbon sources for which the model could not grow in the first place (i.e. there will be NAs for grRatio)
# Also completeDeletions out unassigned/SPONTANEOUS model artifacts
completeDeletions <- completeDeletions %>% filter(grRatio != Inf, grRatio != 'NaN', grRatio != '#NAME?') %>% filter(gene != 'Unassigned', gene != 'SPONTANEOUS', gene != 'unassigned')


essentialDeletions <- completeDeletions %>% mutate(isEssential = as.numeric(grRatio) <= 0.001) %>% 
  ungroup() %>% group_by(gene) %>% mutate(isEverEssential = sum(isEssential)) %>% 
  filter(isEverEssential >= 1)

essentialDeletions$isEssential <- factor(essentialDeletions$isEssential, levels =c(TRUE,FALSE), labels = c('x',''))

essentialDeletions$carbonSource <- factor(essentialDeletions$carbonSource, levels = carbonSourceLevels)

getColors <- brewer_pal('qual') 
myPalette <- getColors(2)
names(myPalette) <- levels(essentialDeletions$strain) # Give every color an appropriate name


FigReviewer3 <- essentialDeletions %>% 
  ggplot(aes(carbonSource, gene, fill = isEssential)) + geom_tile(color = 'grey') + 
   xlab("")+ylab("") + theme_bw()  +
  theme(axis.text.y = element_text(size = 7 ),axis.text.x = element_text(angle = 90,hjust = 1), legend.position = 'none') +
  scale_fill_manual(labels = c("Not Essential", "Essential"),values=c("white","black"))

FigReviewer3

ggsave('For Reviewers/otherPIPlineages.tiff', FigReviewer3,dpi=300)
