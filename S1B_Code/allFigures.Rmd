...
Fig3C <- growthDynamics_Fig3 %>% 
  ...
  xlab("")+ylab("")+ theme_bw() +theme(axis.text.y = element_blank(), legend.position="top", axis.text.x = element_text(angle = 45, hjust = 1), legend.title=element_text(size=8)) +guides(fill = guide_colorbar( title.position = "top",title.hjust = 0.5, title = 'Time Mid-Exponential (hr)'))+
...
pip234dat_raw <- read_csv('For Reviewers/PipDeletionsAllLineages.csv')
...
myPalette <- getColors(2)
names(myPalette) <- levels(essentialDeletions234$strain) # Give every color an appropriate name
...
```{r, echo=FALSE,results=TRUE, warning=FALSE, message=FALSE, fig.align='center', height=8, width=7.5}
# Experimenting with growthcurver instead of my code...
# t_mid -> inflection point...point where we reach half the maximum density
# r -> growth rate
# t_gen -> doubling time (h)
...
# Remake Figure 3B,C,E,F with these newly calculated doubling times and time to half carrying capacity (half theoretical max OD600)
# Load descriptors for these carbon sources 
growthDynamics <- inner_join(growthDynamics,Fig3_CarbonSourceDescriptors)
growthDynamics <- growthDynamics %>% arrange(description)
growthDynamics$carbonSource <- factor(growthDynamics$carbonSource, levels = rev(unique(growthDynamics$carbonSource)),ordered = TRUE)

# Figure 3B - Doubling Time
Fig3B <- growthDynamics %>% 
  filter(carbonSource %in% biologAllGrowth$carbonSource) %>% 
  ggplot(aes(x=strain, y=carbonSource)) +
  geom_tile(aes(fill = t_gen), color='white')+
  xlab("")+ylab("")+theme_bw() +  theme(axis.text.y = element_blank(), legend.position="top", axis.text.x = element_text(angle = 45, hjust = 1), legend.title=element_text(size=8)) + guides(fill = guide_colorbar(title.position = "top",title.hjust = 0.5, title = 'Doubling Time \n (hr)',label.hjust=0.6))+
  scale_fill_gradient(low = "grey95", high = "black", trans='log10', limits=c(.1, 30))+
...
# Figure 3C
Fig3C <- growthDynamics %>% 
  filter(carbonSource %in% biologAllGrowth$carbonSource) %>% 
  ggplot(aes(x=strain, y=carbonSource)) +
  geom_tile(aes(fill = t_mid), color='white')+
  xlab("")+ylab("")+ theme_bw() +theme(axis.text.y = element_blank(), legend.position="top", axis.text.x = element_text(angle = 45, hjust = 1), legend.title=element_text(size=8)) +guides(fill = guide_colorbar( title.position = "top",title.hjust = 0.5, title = 'Time to 1/2 K \n (hr)'))+
  scale_fill_gradient(low = "grey95", high = "black", trans='log10')+
  scale_x_discrete(breaks=c("Day0Anc","Day20C2","Day20F4","Day20P1","Day20T3"),
        labels=c("Ancestor", "LBE", "CIP","PIP","TOB"))

# Figure 3E - Doubling Time
Fig3E <- growthDynamics %>% 
  filter(carbonSource %in% biologAllGrowth$carbonSource) %>% 
  ggplot(aes(x=strain, y = t_gen))+
  geom_boxplot(aes(color = strain), size=1.25)+
  scale_colour_manual(values = c('grey','black','gold2','red','dodgerblue2'),breaks=c("Day0Anc",'Day20C2',"Day20F4","Day20P1","Day20T3"))+
...
Fig3F <- growthDynamics %>% 
  ...
  scale_colour_manual(values = c('grey','black','gold2','red','dodgerblue2'),breaks=c("Day0Anc",'Day20C2',"Day20F4","Day20P1","Day20T3"))+
  theme_bw()+ scale_y_log10() + 
  theme(axis.text.y = element_text(size=10), axis.title.y=element_text(size=12),legend.position="none", axis.text.x = element_blank())+xlab("")+ylab("Time to 1/2 Carrying Capacity (hr)")


# Remake Figure S8 with newly calculated growth dynamics 
