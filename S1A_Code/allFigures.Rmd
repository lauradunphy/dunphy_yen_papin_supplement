...
essentialityPredictions <- essentialityPredictions %>% filter(grRatio != Inf,grRatio != -Inf, grRatio != 'NaN', grRatio != '#NAME?') %>% filter(gene != 'Unassigned', gene != 'SPONTANEOUS', gene != 'unassigned')
...
Fig2A <- Fig2Adat %>% ggplot(aes(carbonSource, strain))+
  # geom_tile(aes(fill=AUCsum), color='black',size = 0.25)+ # should be growth changed to AUCsum to see how it looked
...
Figure2 <- as_ggplot(Figure2) + draw_plot_label(label = c("A","B","C","D"), size = 15, x = c(0, 0, 0.33, 0.67), y = c(1, 0.57,0.57,0.57))
ggsave('Figures and Data/Fig2_AUC.tiff',plot = Figure2, width = 7.5, height = 5, units="in",dpi=400)
...
asterixHeights <- Fig4AMaximums %>% group_by(strain) %>% summarise(h = 0.225) %>% filter(strain != "Day0Anc",strain != "Day20T4")
...
theme_bw()+theme(legend.position = 'none',strip.background = element_rect(fill='white'), axis.title.x = element_blank(), axis.text.x = element_text(angle=45, hjust=1))+
  ...
  asterixHeights2 <- Fig4BMaximums %>% group_by(strain) %>% summarise(h = 0.225) %>% filter(strain != "Day0Anc",strain != "PA14_57880")
 ...
 stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,
  ...
  facet_wrap(~media)+
  ylab("Max Growth (OD600)")+ylim(0,0.24)+
  scale_color_manual(values =c('grey','green2','green2','green2','green2', 'green2','green2','green2','green2'),breaks=c('Day0Anc','PA14_23470','PA14_57570','nuoB','PA14_41710','PA14_44360','PA14_57880','nuoL','PA14_57850'),labels = c('Ancestor','PA14_23470','PA14_57570',expression(italic('nuoB')),'PA14_41710','PA14_44360','PA14_57880',expression(italic('nuoL')),'PA14_57850'))+
theme_bw()+theme(legend.position = 'none',strip.background = element_rect(fill='white'), axis.title.x = element_blank(), axis.text.x = element_text(angle=45, hjust=1))+
  scale_x_discrete(breaks=c('Day0Anc','PA14_23470','PA14_57570','nuoB','PA14_41710','PA14_44360','PA14_57880','nuoL','PA14_57850'),labels = c('Ancestor','PA14_23470','PA14_57570',expression(italic('nuoB')),'PA14_41710','PA14_44360','PA14_57880',expression(italic('nuoL')),'PA14_57850'))
...
# Figure 6CD :
 ...
 # Trying to make Figure 6C points instead of bar graphs
  ...
  signif_heights_6C <- ancP1gnyAMaximums %>% group_by(strain) %>% summarise(h=0.085) %>% filter(strain %in% signif_strains_6C)
...
signif_heights_7 <- Fig7Dat2 %>% group_by(strain) %>% summarise(h=0.058)
...
```{r, echo=FALSE, fig.align='center', fig.height=18, fig.width=7.5, message=FALSE, warning=FALSE}
...
FigureS1A <- biologAllStats %>% 
  ...
  facet_wrap(~carbonSource,15,7,labeller=label_wrap_gen2())+
  ...
  theme_bw()+
  theme(legend.title=element_blank(),legend.position="top",legend.direction = "horizontal", strip.text=element_text(size=6), axis.text = element_text(size=7)) 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=18, fig.width=7.5, fig.align='center'}
...
pip234dat <- pip234dat %>% filter(grRatio != Inf,grRatio != -Inf, grRatio != 'NaN', grRatio != '#NAME?') %>% filter(gene != 'Unassigned', gene != 'SPONTANEOUS', gene != 'unassigned')
